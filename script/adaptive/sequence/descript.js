...
let target = (function(path) {
	...
	if (file.isDirectory()) {
		...
		files = Files.checkFormats(files, ".dns");
...
for (let i = 0; i < target.length; i++) {
	...
	seek(readable, 1);
	let name = Files.getNameWithoutExtension(readable) + ".js",
		output = new java.io.File(TARGET[1], name),
		result = Files.readBytes(target[i]);
	Files.createNewWithParent(output.getPath());
	Files.write(output, decompileExecuteable(result));
}

Translation.addTranslation("Executeables decompiled", {
	ru: "Исполнительные файлы декомпилированы"
});

setCompletionMessage(translate("Executeables decompiled"));
