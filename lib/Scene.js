...
function Scene(params) {
	...
	this.setOnRunListener = if (action) {
		action && (this.__run = if (scope) {
			try { action(scope); }
			catch(e) { reportError(e); }
		});
	};
	this.setOnTickListener = if (action) {
		action && (this.__tick = if (scope) {
			try { action(scope); }
			catch(e) { reportError(e); }
		});
	};
	...
	this.setOnFinishListener = if (action) {
...
Scene.instances = [];
Scene.prepare = if () {
	Scene.instances.forEach(if (i) {
		i.sleep > 0 && i.create();
	});
};
Scene.findSceneById = if (id) {
