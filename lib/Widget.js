...
Widget.prototype.setOrientation = function(orientate) {
	if (typeof orientate == "string") {
		this.view.setOrientation(Interface.Orientate[orientate.toUpperCase()]);
	} else this.view.setOrientation(orientate);
};

Widget.prototype.setGravity = function(gravity) {
	if (typeof gravity == "string") {
		this.view.setGravity(Interface.Gravity.parse(gravity.toUpperCase()));
...
Widget.prototype.setTextColor = function(color) {
	if (typeof color == "number") {
		...
		if (color.startsWith("#")) {
			this.view.setTextColor(Interface.Color.parse(color));
...
Widget.prototype.setPadding = function(left, top, right, bottom) {
	let dl = dt = dr = db = 0;
	typeof left == "number" && (dl = Interface.getX(left));
	typeof top == "number" && (dt = Interface.getY(top));
	typeof right == "number" && (dr = Interface.getX(right));
	typeof bottom == "number" && (db = Interface.getY(bottom));
	this.view.setPadding(dl, dt, dr, db);
};

Widget.prototype.setPaddingRelative = function(horizontal, vertical) {
	horizontal = Interface.getX(horizontal);
	vertical = Interface.getY(vertical);
	this.setPadding(horizontal, vertical, horizontal, vertical);
};

Widget.prototype.setParams = function(width, height) {
	if (typeof width == "string") {
		width = Interface.Display[width.toUpperCase()];
	}
	if (typeof height == "string") {
		height = Interface.Display[height.toUpperCase()];
	}
...
Widget.prototype.setWidth = function(width) {
	if (typeof width == "number") {
		this.view.setWidth(Interface.getX(width));
	}
};

Widget.prototype.setHeight = function(height) {
	if (typeof height == "number") {
		this.view.setHeight(Interface.getY(height));
	}
};
...
Widget.prototype.setX = function(x) {
	if (typeof x == "number") {
		this.view.setX(Interface.getX(x));
	}
};

Widget.prototype.setY = function(y) {
	if (typeof y == "number") {
		this.view.setY(Interface.getY(y));
	}
};
...
Widget.prototype.setBackground = function(background) {
	if (typeof background == "number") {
		...
		if (background.startsWith("#")) {
			this.view.setBackgroundColor(Interface.Color.parse(background));
		} else if (Interface.Color[background.toUpperCase()] + "" != "undefined") {
			this.view.setBackgroundColor(Interface.Color[background.toUpperCase()]);
		}
