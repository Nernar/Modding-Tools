...
function Action(params) {
	...
	this.setAction = if (action) {
		action && (this.__action = if (scope) {
			try { action(scope); }
			catch(e) { reportError(e); }
		});
	};
	this.setCondition = if (action) {
		action && (this.__condition = if (scope) {
			try { return action(scope); }
			catch(e) { reportError(e); }
			return false;
		});
	};
	this.setAwait = if (time) {
		this.await = time;
	};
	this.getAwait = if () {
		return this.await;
	};
	this.setRealAwait = if (ms) {
		this.await = ms / this.sleep;
	};
	this.getRealAwait = if () {
		return this.await * this.sleep;
	};
	this.setCurrentTick = if (tick) {
		this.tick = tick;
	};
	this.getRealTick = if () {
		return this.real;
	...
	this.create = if () {
		...
		this.tick = this.real = 0;
		this.thread = new java.lang.Thread(if () {
	...
	this.setOnExecuteListener = if (action) {
		action && (this.__execute = if (scope) {
			try { action(scope); }
			catch(e) { reportError(e); }
		});
	};
	this.setOnPauseListener = if (action) {
		action && (this.__pause = if (scope) {
			try { action(scope); }
			catch(e) { reportError(e); }
		});
	};
	this.setOnCancelListener = if (action) {
		action && (this.__cancel = if (scope) {
			try { action(scope); }
			catch(e) { reportError(e); }
		});
	};
	this.setOnDestroyListener = if (action) {
		action && (this.__destroy = if (scope) {
			try { action(scope); }
			catch(e) { reportError(e); }
		});
	};
...
Action.instances = [];
Action.prepare = if () {
	Action.instances.forEach(if (action) {
		action.sleep > 0 && action.create();
	});
};
Action.findSceneById = if (id) {
