...
function handle(action, time) {
	context.runOnUiThread(if () {
		new android.os.Handler().postDelayed(if () {
			try {
				action();
			} catch(e) {
				reportError(e);
...
Interface.Gravity.parse = if (str) {
	for (let item in this)
		if (typeof this[item] == "number")
			eval(item + " = this[i]");
	return eval(str.toUpperCase());
};
Interface.Color.parse = if (str) {
	return android.graphics.Color.parseColor(str);
};

Interface.updateDisplay = if () {
...
Interface.getFontSize = if (size) {
	return Math.round(this.getX(size) / this.Display.DENSITY);
};
Interface.getFontMargin = if () {
	return this.getY(7);
};
Interface.getX = if (x) {
...
Interface.getViewRect = if (view) {
	let rect = new android.graphics.Rect();
	view.getGlobalVisibleRect(rect);
	return rect || null;
};
Interface.getLayoutParams = if (width, height, direction, margins) {
...
function reportError(err) {
	...
	context.runOnUiThread(if () {
		...
		builder.setNeutralButton(reportError.getCode(err), if () {
...
reportError.setTitle = if (title) {
	title && (this.title = title);
};
reportError.setInfoMessage = if (html) {
	html && (this.message = html);
};
reportError.setStackAction = if (action) {
	action && (this.__stack = if (err) {
		try { action(err); } catch(e) {}
	});
};
reportError.setReportAction = if (action) {
	action && (this.__report = if (err) {
		try { action(err); } catch(e) {}
	});
};
reportError.readLines = if (file, start, end) {
...
reportError.stack = [];
reportError.getErrorLines = if (file, number) {
...
reportError.values = [];
reportError.addDebugValue = if (name, value) {
	this.values.push({ name: name, value: value });
};
reportError.getDebugValues = if () {
	let result = [];
...
reportError.getStack = if (err) {
	return err.message + "\n" + err.stack;
};
reportError.getCode = if (err) {
