...
Network.prototype.connect = function() {
	...
	else throw Error("Can't find any opened connection to connect");
...
Network.Reader.prototype.read = function() {
	...
	if (!stream) {
		throw Error("Can't read stream, because one of params is missing");
	}
...
Network.Writer.prototype.getOutputStream = function() {
	throw Error("You must overwrite a method getOutputStream()");
};
...
Network.Writer.prototype.download = function() {
	...
	if (!stream) {
		throw Error("Can't download stream, because input stream is missing");
	}
	if (!output) {
		throw Error("Can't download stream, because output stream is missing");
	}
...
Network.handle = function(action, callback, connect) {
	if (!(action instanceof Function)) {
		throw Error("Nothing to network handle");
	}
