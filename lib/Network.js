...
Network.Reader.prototype.read = function() {
	...
	if (!stream) {
		throw "Can't read stream, because one of params is missing";
	}
...
Network.Writer.prototype.getOutputStream = function() {
	throw "You must overwrite a method getOutputStream()";
};
...
Network.Writer.prototype.download = function() {
	...
	if (!stream) {
		throw "Can't download stream, because input stream is missing";
	}
	if (!output) {
		throw "Can't download stream, because output stream is missing";
	}
...
Network.handle = function(action, callback, connect) {
	if (!(action instanceof Function)) {
		throw "Nothing to network handle";
	}
