function CoordsPopup() {
	...
	this.addButtonElement = function(name, click) {
		let views = this.views, index = views.buttons.length;
	...
	this.addGroup = function(name) {
		let views = this.views, mathes = this.mathes, index = views.groups.length;
		...
		return this.groups[index] = {
			...
			updateMather: function(index) {
				let item = this.items[index], current = item.mathes[item.current[1]];
		...
		for (let i in this.groups)
			if (this.groups[i].onLongChange)
				for (let f in this.groups[i].items) {
					let item = this.groups[i].items[f], group = this.groups[i];
