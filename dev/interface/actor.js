...
WindowActor.prototype.setInterpolator = function(interpolator) {
	if (interpolator instanceof ActorInterpolator) {
		if (interpolator.TYPE == "none") {
			throw new Error("WindowActor can't use empty interpolator");
		}
...
WindowActor.prototype.setPathMotion = function(motion) {
	if (motion instanceof ActorPathMotion) {
		if (motion.TYPE == "none") {
			throw new Error("Abstract class ActorPathMotion can't be reached");
		}
...
WindowActor.prototype.setPropagation = function(propagation) {
	if (propagation instanceof ActorPropagation) {
		if (propagation.TYPE == "none") {
			throw new Error("Abstract class ActorPropagation can't be reached");
		}
...
ActorScene.prototype.reset = function(root, container) {
	if (root && container) {
		...
		this.scene = new android.transition.Scene();
	}
};
...
ActorManager.prototype.setActor = function(sceneFromOrTo, sceneToOrActor, actor) {
	if (sceneFromOrTo instanceof ActorScene) {
		sceneFromOrTo = sceneFromOrTo.getScene();
	}
	if (sceneToOrActor instanceof WindowActor) {
		sceneToOrActor = sceneToOrActor.getActor();
	} else if (sceneToOrActor instanceof ActorScene) {
...
ActorManager.prototype.actorTo = function(scene) {
	if (scene instanceof ActorScene) {
...
ActorManager.beginDelayedActor = function(container, actor) {
	if (!container) {
		throw new Error("Unable to use delayed actor on undefined");
	}
	if (actor && actor instanceof WindowActor) {
		actor = actor.getActor(); 
	}
...
ActorManager.endActors = function(container) {
	if (!container) {
		throw new Error("Can't end actors without container");
	}
	android.transition.TransitionManager.endTransitions(container);
};
ActorManager.go = function(scene, actor) {
	if (!container) {
		throw new Error("ActorManager.go can't be resolved without ActorScene");
	}
	if (actor && actor instanceof WindowActor) {
		actor = actor.getActor();
	}
