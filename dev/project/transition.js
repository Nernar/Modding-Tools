function TransitionWorker(obj) {
	...
	this.Animation = {
		...
		getFrameCount: function() {
			var count = 0;
			for (var i = 0; i < this.getAnimateCount(); i++)
		...
		setParams: function(params) {
			...
			for (var i = 0; i < params.length; i++) {
				var animate = this.createAnimate();
		...
		Animate: function() {
			...
			this.setupFrame = function(index) {
				var real = this.getFrameCoords(index), frame = this.getFrame(index),
			...
			this.getFrameCoords = function(index) {
				var point = new Object(), frames = this.getFrames(),
