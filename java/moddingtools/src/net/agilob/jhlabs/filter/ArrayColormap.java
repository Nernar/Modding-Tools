...
import net.agilob.jhlabs.filter.math.ImageMath;
...
public class ArrayColormap implements Colormap, Cloneable {
     ...
     *         the position to set
     * @param firstIndex
     *         the position of the first color from which to interpolate
     * @param lastIndex
     *         the position of the second color from which to interpolate
     * @param color
     *         the color to set
     */
    public void setColorInterpolated(int index, int firstIndex, int lastIndex, int color) {
        ...
        for(int i = firstIndex; i <= index; i++)
            map[i] = ImageMath.mixColors((float) (i - firstIndex) / (index - firstIndex),
                                         firstColor, color);
        for(int i = index; i < lastIndex; i++)
            map[i] = ImageMath.mixColors((float) (i - index) / (lastIndex - index), color,
                                         lastColor);
    }
     ...
     * @param lastIndex
     *         the position of the second color
     * @param color1
     *         the first color
     * @param color2
     *         the second color
    ...
    public void setColorRange(int firstIndex, int lastIndex, int color1, int color2) {
        for(int i = firstIndex; i <= lastIndex; i++)
            map[i] = ImageMath.mixColors((float) (i - firstIndex) / (lastIndex - firstIndex),
                                         color1, color2);
    }
